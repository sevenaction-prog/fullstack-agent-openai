...
# Use the current official Android CPU-variant strategy from llama.cpp.
python3 "$ROOT/scripts/patch_llama_android.py"

# Mobile-safe defaults for the Fold6: lower context and batch to reduce peak RAM.
sed -i 's/DEFAULT_CONTEXT_SIZE    = 8192/DEFAULT_CONTEXT_SIZE    = 2048/' "$ROOT/third_party/llama.cpp/examples/llama.android/lib/src/main/cpp/ai_chat.cpp"
sed -i 's/BATCH_SIZE              = 512/BATCH_SIZE              = 128/' "$ROOT/third_party/llama.cpp/examples/llama.android/lib/src/main/cpp/ai_chat.cpp"
